...
const stage = {
    ...
    doAttack(attacking, attacked) {
        ...
        if (actualAttack > actualDefense) {
            ...
            log.addMessage(`${attacking.name} atacou ${attacked.name} causando ${actualAttack.toFixed(2)} de dano.`);
        } else {
            log.addMessage(`${attacking.name} atacou ${attacked.name} mas não causou dano.`);
        }
...
const log = {
    ...
    render() {
        ...
        for(let item in this.list) {
            logEl.innerHTML += `<li>${this.list[item]}</li>`;
        }
    }
}